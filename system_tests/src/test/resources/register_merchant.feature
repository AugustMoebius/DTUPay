Feature: Register a merchant in DTUPay
  ...
  Scenario: Register a merchant with wrong CVR
    ...
    And the merchant has an account in the DTU bank with a balance at 200
    When the merchant submits a request to register
    Then the merchant submission fails and he gets an error message "Illegal registration: Invalid CVR number. Received DK11111112."
  ...
  Scenario: Register a merchant with wrong first name
    ...
    And the merchant has an account in the DTU bank with a balance at 200
    When the merchant submits a request to register
    Then the merchant submission fails and he gets an error message "Illegal registation: Invalid name. Recieved La9rs."

  Scenario: Register a merchant with wrong first name
    Given a merchant with the CVR number "DK11111112", with the name "Lars" "La3rsen"
    And the merchant has an account in the DTU bank with a balance at 200
    When the merchant submits a request to register
    Then the merchant submission fails and he gets an error message "Illegal registation: Invalid name. Recieved La3rsen."
  ...
  Scenario: Register a merchant which already exists in the system
    ...
    And the merchant has an account in the DTU bank with a balance at 200
    When the merchant submits a request to register
    Then the merchant submission fails and he gets an error message "Illegal registration: Merchant already exists."
